...
- Setup to analyze and browse the source-code of The Rust Programming Language itself [from Emacs](https://github.com/DerickEddington/dotfiles/blob/c71be1fa7f332cd248dfcbcc805a3f3ae102c3c5/.config/emacs/my/init/rust.el#L47).  This is especially useful for studying or debugging The Rust Standard Libraries (either directly or when stepping into a custom toolchain's used by a project).  This requires [special configuration](https://github.com/DerickEddington/dotfiles/blob/c71be1fa7f332cd248dfcbcc805a3f3ae102c3c5/.config/emacs/my/lib/my-rust-lang-proj.el#L22) of `lsp-mode` and `rust-analyzer` (similar to what is done for VS Code to do this).  This works best with [a few tweaks](https://github.com/DerickEddington/rust/compare/my-exploring%5E..my-exploring) to a `rust-lang/rust` directory.
...
- Currently supports: `FreeBSD`, `Linux/Alpine`, `Linux/Debian`, `Linux/NixOS`, `Linux/Ubuntu`,
  `NetBSD`, and `SunOS/OpenIndiana`.  Further support for other POSIX OSs can be added in the
  future.
...
- When in NixOS, uses [KeePassXC](https://keepassxc.org/) for automatic entry of: [website](https://github.com/DerickEddington/nixos-config/blob/5908adb5dc43ba2dbb1e4af7344bff6247fbf2c8/firefox.nix#L50) [passwords](https://github.com/DerickEddington/dotfiles/blob/1c23cc4f79ef914eda6d5ed1b89cd38d5c401a45/.config/home-manager/common/default.nix#L145); SSH key passphrases and [adding to `ssh-agent`](https://github.com/DerickEddington/dotfiles/blob/1c23cc4f79ef914eda6d5ed1b89cd38d5c401a45/.config/keepassxc/keepassxc.ini#L40); and as the [Secret Service (D-Bus API)](https://github.com/DerickEddington/dotfiles/blob/1c23cc4f79ef914eda6d5ed1b89cd38d5c401a45/.config/keepassxc/keepassxc.ini#L19) (instead of [GNOME Keyring](https://github.com/DerickEddington/nixos-config/blob/b753aa92a0a59e81e09b34ee559921459c6561a4/secret-service.nix#L32) or KDE Wallet) which is further used for: Git [credentials](https://github.com/DerickEddington/nixos-config/blob/b753aa92a0a59e81e09b34ee559921459c6561a4/secret-service.nix#L61); NetworkManager (wifi, VPN) passwords; GnuPG key passphrases (via the `pinentry` program); [etc](https://github.com/DerickEddington/nixos-config/blob/5908adb5dc43ba2dbb1e4af7344bff6247fbf2c8/nixpkgs/overlays.nix#L73).  Premade template DB and browser settings, to help a new user [get started](https://github.com/DerickEddington/dotfiles/blob/main/.local/share/my/keepass/README.md) with this consistently.
