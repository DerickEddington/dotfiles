...
;; limit of `sockaddr_un.sun_path', which is why this uses short code-names for sub-directories.)
;;
                      ...
                      (cons name
                            (mapcar #'(lambda (p)
                                        (let ((key (car  p)) (val (cadr p)))
                                          (cond
                                           ;; Change the login program to `env', to enable setting
                                           ;; our special environment variable.
